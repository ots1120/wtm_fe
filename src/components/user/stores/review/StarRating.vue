<template>
    <div class="flex items-end justify-center mb-4 gap-4">
        ...
        <div class="flex text-yellow-400 w-28" @mouseleave="clearHover">
            <!-- 각 별점의 input과 label -->
            <input v-for="star in 5" :key="star" type="radio" :id="`${category}-${star}-stars`" :name="category"
                :value="star" v-model="rating" class="hidden" />
            <label v-for="star in 5" :key="star" :for="`${category}-${star}-stars`"
                class="cursor-pointer text-gray-300 transition"
                :class="{ 'text-yellow-400': star <= hoverRating || star <= rating }" @mouseenter="setHover(star)">
                &#9733;
            </label>
        </div>
    </div>
