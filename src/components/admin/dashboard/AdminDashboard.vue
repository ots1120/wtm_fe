<template>
  <div class="m-5 flex flex-col items-center">
    <div>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-24" viewBox="0 0 16 16">
        <path
          fill="#BDBDBD"
...
export default {
  ...
  methods: {
    goToMenuManagement() {
      this.$router.push(`/admin/${this.storeId}/menu`); // storeId를 라우팅에 반영
    },
    goToTicketManagement() {
      this.$router.push(`/admin/${this.storeId}/ticket`);
    },
    goToReviewManagement() {
      this.$router.push(`/admin/${this.storeId}/review`);
    },
    goToNoticeManagement() {
      this.$router.push(`/admin/${this.storeId}/notice`);
    },
    goToStoreManagement() {
