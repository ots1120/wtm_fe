  ...
  <div>
    ...
    >
      ...
      <div class="w-full flex-col p-3">
        ...
        <!-- 답글 버튼 -->
        <div v-if="review.reply.length <= 0" class="flex justify-end">
          <button
...
<script>
import ReplyForm from '@/components/admin/review/ReplyForm.vue';
...
export default {
  ...
  data() {
    return {
      isReplying: Array(this.reviews.length).fill(false), // 답글 상태 저장
      isExpanded: Array(this.reviews.length).fill(false), // 텍스트 확장 상태 저장
    };
  },
  methods: {
    toggleReply(index) {
      this.isReplying[index] = !this.isReplying[index];
    },
    toggleExpand(index) {
      this.isExpanded[index] = !this.isExpanded[index];
    },
    submitReply(reviewId, replyContent) {
      console.log(`Review ID: ${reviewId}, Reply: ${replyContent}`);
      // 여기에 답글 저장 로직 추가
    },
  },
  watch: {
    reviews(newReviews) {
      this.isReplying = Array(newReviews.length).fill(false);
      this.isExpanded = Array(newReviews.length).fill(false);
    },
  },
};
</script>
